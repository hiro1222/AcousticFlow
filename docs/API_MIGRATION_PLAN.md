...
### 段 3: ホストから A 分類を削除（**固まったものだけ**）
- **移す**（挙動が安定しており、調整の必要がないもの）:
  レート管理カウンタ 5 種 / 音源ループ / エッジカタログ判断 / `BuildMainSourceTaps` の骨格。
- **移さない**（まだ調整中。DEV_LOG E-7「固まってから移植する」に従いホストに残す）:
  体積推定・mixing time(√V)・残響目標比・知覚圧縮の指数・尾 IR の平滑パラメータ。
  → **§3-1 の卒業条件を満たしてから、別途移す**。
- ホスト側の該当コードを削除。
- **確認**: 同じ音。行数の削減を実測して記録。

#### 実施結果（2026-07-30）— 見積もりは外れた

**A 分類の除去自体は完了**（計算系の旧クエリはホストから消え、レート管理も移った）。
一方 **行数はほぼ減らなかった**（1576 → 1585 行。コメント増で微増）。

当初「1500 → 700 行台」と見積もったが、**これは誤りだった**。実測の内訳:

| 分類 | 実測 | 判定 |
|---|---|---|
| Inspector 公開フィールド + Tooltip | 約 98 行 | B: 残す（ホストの設定 UI） |
| Wwise 出力系（`AcousticEngine.*` 呼び出し） | 約 63 行 | B: 残す（ホスト固有の出力先） |
| GUI / Gizmo / Monitor 公開 | 約 176 行 | B: 残す（可視化はホストの仕事） |
| 入力 / 移動 | 約 24 行 | B: 残す |
| タップ合成・空気吸収・距離減衰 | 約 90 行 | C: 共通ライブラリ候補。**HRTF で変わるので今は動かさない** |
| 体積推定 / mixing time / 知覚圧縮 | 約 42 行 | §3-1 により残す |

**誤りの原因**: ホストの大半が最初から B 分類（Unity 入力・Wwise 出力・GUI・Gizmo）で、
A 分類（エンジンの知識）は元々少なかった。「1500 行あるから 800 行削れる」は
中身を見ずに出した数字であり、棚卸しの結論（B は残すべき）と矛盾していた。

**評価をどう改めるか**: 移行の成果は行数ではなく**「移植時に書き直す量」**で測るべき。
その観点では A 分類が消えたので目的は達している。行数は B が主なので減らないのが正しい。

### 3-1. C++ へ移す卒業条件（原則: DEV_LOG E-7）

反復が安い場所（C# ホスト）で試し、**固まってから** C++ へ移す。「固まった」の判定:

1. **4 部屋すべてで耳で妥当**と確認できている（小 / 中 / 大 / 特大）
2. **直近 1 セッションで係数を触っていない**
3. **物理量である**（知覚に合わせた補正値はホスト側 or 共通ホスト支援層が妥当。§2 未確定 3）
4. **数値回帰テストがある**（移した後に壊れたら検出できる）

現時点の判定:

| 対象 | 状態 | 判定 |
|---|---|---|
| レート管理 / 音源ループ / エッジカタログ判断 | 長期間安定 | **移す** |
| `BuildMainSourceTaps` の骨格 | 6 帯域化以降は安定 | **移す** |
| 距離減衰の適用位置（音源→反射点） | 物理として決着済み・エンジン側に実装済み | **済** |
| 体積推定 / mixing time √V | 当日追加。√V は経験則で未検証 | **残す** |
| 残響目標比 (r/r_c)² | 当日 4 回書き直し | **残す** |
| 知覚圧縮の指数 | 当日追加・未検証。かつ物理量でない | **残す**（恒久的にホスト側かも） |
| 尾 IR の平滑パラメータ群 | 調整の余地が大きい | **残す** |
...
| C++ 側の回帰を検出できない（テストが 342 行しかない） | **段 0 として先に数値回帰テストを足す**（決定済み）。既知シーンでの不変量を固定し、段ごとに実行する |
| 調整中のものを C++ へ移してしまい、反復コストが跳ね上がる | **§3-1 の卒業条件**で判定する。DEV_LOG E-7 の原則を計画に明文化した |
